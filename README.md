...
* Please do **NOT** use the server to store your data. We have ordered a workstation and you can save your data there.
...
1. `app.py` is your program as shown below. It prints out some messages, and write a string to file named `/data/tutorial.txt`.
...
1. Build docker image. Under this tutorial folder, run
  ...
  ```
  `--tag=` is the docker image name

1. The docker image is installed in you host machine already. To list your docker images, run
```
docker image ls
```
...
1. Click the *Logs* under *Quick actions*, you can see the output
  ...
  <img src="./images/log_docker.png" width=400 />
